...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let camera_video_hub = mqtt_client
        .create_broadcast_hub("security/camera/front_door/mjpeg", QoS::AtMostOnce, 64)
        .await?;

    let camera_telemetry_hub = mqtt_client
        .create_broadcast_hub("security/camera/front_door/events", QoS::AtMostOnce, 64)
        .await?;
...
<body style="background:#111; color:#eee; font-family:sans-serif; text-align:center; padding:2rem;">
  <h2>📹 MQTT Live Security Camera Feed</h2>
  <img src="/api/camera/mjpeg" style="border:2px solid #444; border-radius:8px; width:640px; height:480px; background:#000;" />
  <div id="events" style="margin-top:1rem; color:#4ade80;">Waiting for AI motion events...</div>
  <script>
    const es = new EventSource('/api/camera/events');
    es.onmessage = (e) => {
    ...
    println!("[*] Server running. Ingesting and broadcasting live camera streams...");
    tokio::time::sleep(Duration::from_millis(500)).await;
    println!("[*] Demo complete! Clean shutdown.");
